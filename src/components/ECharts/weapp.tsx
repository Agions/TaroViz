...
import { WeappAdapterOptions } from './types/platform';
import { DataURLOption, LoadingOption } from './types/common';

// 导入样式
import './styles/weapp.scss';

// 微信小程序环境的ECharts组件
const WeappECharts = forwardRef<EChartsRef, EChartsProps>((props, ref) => {
  const {
    option,
    width = '100%',
    height = '300px',
  ...
  // 创建适配器引用
  const adapterRef = useRef<WeappAdapter | null>(null);
  // 图表实例
  ...
  useEffect(() => {
    ...
    const adapterOptions = {
      ...
      onEvents
    } as WeappAdapterOptions;

    // 创建适配器实例
    adapterRef.current = new WeappAdapter(adapterOptions);

    // 初始化图表
    adapterRef.current.init().then(instance => {
      if (instance) {
        setChartInstance(instance);
      }
    });
        ...
        query.select(`#${canvasId}`)
          .boundingClientRect(rect => {
            if (rect && typeof rect === 'object' && 'width' in rect && 'height' in rect) {
              onResize(rect.width as number, rect.height as number);
            }
          })
          .exec();
      }
...
// 设置组件名称
WeappECharts.displayName = 'WeappECharts';

export default WeappECharts;
