...
/**
 * 微信小程序适配器类
 * 实现Adapter接口，提供小程序环境下的图表功能
 */
class WeappAdapter implements Adapter {
  private instance: echarts.ECharts | null = null;
  private options: WeappAdapterOptions;
  private chartCanvas: any = null;
  private isCanvasReady: boolean = false;

  constructor(options: WeappAdapterOptions) {
    this.options = options;
    // 确保onInit被正确处理
    if ('onInit' in options && typeof options.onInit === 'function') {
      this.options.onInit = options.onInit;
    }
  }
   ...
   * 转换为dataURL
   */
  async convertToDataURL(_opts?: DataURLOption): Promise<string | undefined> {
    if (!this.instance || !this.isCanvasReady) {
      return undefined;
    }
    ...
    try {
      ...
      console.error('Failed to convert to dataURL:', error);
      return undefined;
    }
  }
   ...
   * 获取图表的图片URL
   */
  async getDataURL(_opts?: DataURLOption): Promise<string | undefined> {
    return this.convertToDataURL(_opts);
  }
