/**
 * TaroViz 企业微信小程序适配器
 * 基于企业微信小程序canvas组件实现图表渲染
 */
import { Canvas } from '@tarojs/components';
import Taro from '@tarojs/taro';
import { Adapter, WeappAdapterOptions, EChartsOption, EventHandler } from '@agions/taroviz-core/types';
import { uuid } from '@agions/taroviz-core/utils';
import * as echarts from 'echarts/core';
import React from 'react';

// 为WeappAdapterOptions添加选项属性（临时解决lint错误）
interface QYWXAdapterOptions extends WeappAdapterOptions {
  option?: EChartsOption;
  style?: React.CSSProperties;
}

/**
 * 企业微信小程序环境下的ECharts适配器
 */
class QYWXAdapter implements Adapter {
  private instance: any = null;
  private options: QYWXAdapterOptions;
  private canvasId: string;
  private ctx: any = null;
  private component: any = null;
  private canvasDom: any = null;

  constructor(options: QYWXAdapterOptions) {
  ...
  convertToDataURL(opts?: any): string | undefined {
    // 小程序环境不支持直接获取DataURL
    console.warn('[TaroViz] convertToDataURL not supported in QYWX Mini Program environment');
    return undefined;
  }
  ...
  getDataURL(opts?: any): string | undefined {
    // 小程序环境不支持直接获取DataURL
    console.warn('[TaroViz] getDataURL not supported in QYWX Mini Program environment');
    return undefined;
  }
  ...
  render(): JSX.Element {
    ...
    return React.createElement(Canvas, {
      ...
      id: this.canvasId,
      style: mergedStyle,
      className: 'taroviz-echarts-qywx',
    });
  }
}

// 导出适配器
export default QYWXAdapter;
