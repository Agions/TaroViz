...
import { Adapter, MiniAppAdapterOptions, EChartsOption, EventHandler } from '@agions/taroviz-core/types';
 ...
 * 钉钉小程序环境下的ECharts适配器
 */
class DDAdapter implements Adapter {
  private instance: any = null;
  private options: DDAdapterOptions;
  ...
  convertToDataURL(opts?: any): string | undefined {
    // 小程序环境不支持直接获取DataURL
    console.warn('[TaroViz] convertToDataURL not supported in DD Mini Program environment');
    return undefined;
  }
  ...
  getDataURL(opts?: any): string | undefined {
    // 小程序环境不支持直接获取DataURL
    console.warn('[TaroViz] getDataURL not supported in DD Mini Program environment');
    return undefined;
  }
